...
* **Módulo de Pagamentos e Estoque**:
  - `POST /pagamentos/`: Simula a liquidação financeira.
...
| ID | Funcionalidade / Cenário | Ação (Endpoint) | Status Esperado | Validação |
|:---|:---|:---|:---:|:---:|
| **T01** | Login Válido | `POST /auth/login` | `200 OK` | Aprovado |
| **T02** | Acesso Restrito sem Token | `GET /pedidos/` | `401 Unauthorized` | Aprovado |
| **T03** | Acesso a Rota Pública (Produtos) | `GET /produtos/` | `200 OK` | Aprovado |
| **T04** | Cadastro de Produto sem Permissão | `POST /produtos/` | `401 Unauthorized` | Aprovado |
| **T05** | Criação de Pedido sem Campo Obrigatório | `POST /pedidos/` | `422 Unprocessable` | Aprovado |
| **T06** | Criação de Pedido Válido (Baixa Estoque) | `POST /pedidos/` | `201 Created` | Aprovado |
| **T07** | Pedido de Produto Inexistente | `POST /pedidos/` | `404 Not Found` | Aprovado |
| **T08** | Integração de Pagamento Válida | `POST /pagamentos/` | `200 OK` | Aprovado |
| **T09** | Falha no Pagamento (Ex: Inválido) | `POST /pagamentos/` | `400 Bad Request` | Aprovado |
| **T10** | Filtro de Pedidos por Canal | `GET /pedidos/?canalPedido=`| `200 OK` | Aprovado |
| **T11** | Validação de Estoque Insuficiente | `POST /pedidos/` | `409 Conflict` | Aprovado |
